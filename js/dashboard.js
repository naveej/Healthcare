...
foodForm.addEventListener('submit', (event) => {
  ...
  fetch(`https://api.nutritionix.com/v1_1/search/${foodInput}?fields=item_name%2Cnf_total_fat%2Cnf_total_carbohydrate%2Cnf_protein&appId=eb69f562&appKey=6abfb6d1e676f6d3dde0e5156547afbf`)
    ...
    //   const nutrientData = food.fields.nutrient_values;
      ...
      const nutrientDensity = calculateNutrientDensity(food.fields.nf_total_fat, food.fields.nf_total_carbohydrate, food.fields.nf_protein);
      const nutrientDistribution = calculateNutrientIntakeDistribution(nutrientDensity);
    ...
    .catch(error => {
      nutrientResults.innerHTML = '<p>An error occurred while retrieving nutrient data.</p>';
      console.error(error);
    });
});

function calculateNutrientDensity(totalFAT, totalCarbohydrate, protein) {
  const calorieDensity = (totalFAT * 9 + totalCarbohydrate * 4 + protein * 4) / 100;
...
//  const ENERC_KCAL =calculateNutrientDensity.totalFAT* 9 + calculateNutrientDensity.totalCarbohydrate * 4 + calculateNutrientDensity.protein * 4
  ...
  return { FAT: FAT / total * 100, CHOCDF: CHOCDF / total * 100, PROCNT: PROCNT / total * 100 };
}
...
//   console.log(nutrientIntakeDistribution);
  ...
  if (nutrientIntakeDistribution.FAT < targetNutrients.FAT) {
   // console.log('hello');
    ...
    fetch(`https://api.edamam.com/search?q=&app_id=${APP_ID}&app_key=${APP_KEY}&diet=balanced`)
      .then(response => response.json())
      .then(data => {
        //console.log(data);
          ...
          let html = `
                    <div class = "meal-item">
                        ...
                        <div class = "meal-name">
                            ...
                            <a href = "${data.hits[i].recipe.url}" class = "recipe-btn">Get Recipe</a>
                        </div>
                    </div>
                `;
  ...
  if (nutrientIntakeDistribution.CHOCDF < targetNutrients.CHOCDF) {
    ...
    fetch(`https://api.edamam.com/search?q=&app_id=${APP_ID}&app_key=${APP_KEY}&diet=high-fiber`)
      .then(response => response.json())
      .then(data => {
        for (let i = 0; i < data.hits.length; i++) {
          const recipe = data.hits[i].recipe;
  ...
  if (nutrientIntakeDistribution.PROCNT < targetNutrients.PROCNT) {
    ...
    fetch(`https://api.edamam.com/search?q=&app_id=${APP_ID}&app_key=${APP_KEY}&diet=high-protein`)
      .then(response => response.json())
      .then(data => {
      // console.log(data.hits);
        for (let i = 0; i < data.hits.length; i++) {
